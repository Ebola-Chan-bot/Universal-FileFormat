...
namespace 万能文件格式
{
	...
	static 分配块 搜索空隙(_文件头* 文件头指针, uint64_t 新块索引, uint64_t 字节数)noexcept
	{
		...
		if (下块指针->偏移 >= 新块值.偏移 + 新块值.大小)
			...
			for (;;)
			{
				...
				{
					下块指针->下块 = 新块索引;
					新块值.上块 = 分配块区[下块指针->上块].下块;
					新块值.下块 = 无效值;
					break;
				}
				分配块& 下下块引用 = 分配块区[下块指针->下块];
				if (下下块引用.偏移 >= 新块值.偏移 + 新块值.大小)
				{
					新块值.上块 = 下下块引用.上块;
					新块值.下块 = 下块指针->下块;
					下下块引用.上块 = 下块指针->下块 = 新块索引;
					break;
				}
				下块指针 = &下下块引用;
			}
	...
	static void 空隙先耗尽(指针分配器* 分配器, 分配块& 新块值, uint64_t 新块索引, uint64_t 原本分配块个数)
	{
	...
	static void 分配块区扩张(指针分配器* 分配器, uint64_t 新块索引, uint64_t 字节数)
	{
		...
		分配块* 需挪首块指针 = 分配块区 + 新块值.上块;
		新块值.下块 = 需挪首块指针->下块;
		for (;;)
		{
			if (新块值.下块 == 无效值)
			{
				空隙先耗尽(分配器, 新块值, 新块索引, 原本分配块个数);
				return;
			}
		...
		分配块* 中间块指针 = 分配块区 + (需挪首块指针->上块 = 寻隙块指针->上块);//寻隙块前方有空间，因此寻隙块的上块将变成需挪块的上块。中间块设为需挪块的上块，也就是当前空隙之前那块
		uint64_t 寻隙块索引 = 中间块指针->下块;//中间块的下块就是寻隙块，缓存起来。在空隙耗尽之前不会改变。
	...
	void 指针分配器::分配(uint64_t 句柄, uint64_t 字节数)
	{
		if (句柄 < _文件头指针->分配块个数)
		{
			...
			{
				if (_文件头指针->分配块区[块值.下块].偏移 < 块值.偏移 + 字节数)
				{
