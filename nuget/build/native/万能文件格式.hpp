...
namespace 万能文件格式
{
	...
	{
		std::iostream& 基础流;
		//如果基础流支持写出，则必须支持随机访问和超范围的写出并扩展操作。此对象使用期间，基础流必须保持有效。如果流不支持写出，初始化、分配、写出和释放将导致基础流实现定义的行为。
		constexpr 流式分配器(std::iostream& 基础流) :基础流(基础流) {}
		//指示文件中现有内容是否应当被忽略，创建新的空文件。构造完成后必须先调用此方法才能使用任何其它功能，否则导致未定义行为。如果试图打开而非新建一个无效的文件，将抛出std::domain_error。
		void 初始化(bool 新建);
		//分配一个空闲句柄，具有指定的空间
		uint64_t 分配(uint64_t 字节数);
		//为指定句柄分配空间。如果句柄已经有空间，将会保留原有数据，除非新空间比原空间小，将导致数据被截断。如果新空间更大，在保留原有数据的同时，多余的空间将填充未定义的数据。避免使用不必要的过大句柄值，将导致空间浪费。
